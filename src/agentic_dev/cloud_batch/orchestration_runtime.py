    ...
    for wave in commit_waves:
        ...
        for item_id in wave.item_ids:
            ...
            try:
                ...
                _propagate_dependency_blocks(item_map)
                batch = _persist_batch(project_path, batch, plan, item_map, item_results, now_factory=now_factory, event_type="batch_apply_item_failed", details={"item_id": item_id, "wave_id": wave.wave_id, "error_type": type(error).__name__}, persist=not dry_run)
    ...
    for group in groups:
        ...
        for item_id in group.item_ids:
            ...
            try:
                ...
                item_map[item_id] = _failed_item(item, error)
                batch = _persist_batch(project_path, batch, plan, item_map, item_results, now_factory=now_factory, event_type="batch_resume_item_failed", details={"item_id": item_id, "group_id": group.wave_id, "error_type": type(error).__name__}, persist=True)
        if group_failed:
            _propagate_dependency_blocks(item_map)
            batch = _persist_batch(project_path, batch, plan, item_map, item_results, now_factory=now_factory, event_type="batch_resume_group_failed", details={"group_id": group.wave_id}, persist=True)
...
def _propagate_dependency_blocks(item_map: dict[str, BatchItem]) -> None:
    ...
    while changed:
        ...
        for item_id in ordered_ids:
            ...
            if item.status in TERMINAL_ITEM_STATUSES:
                continue
            blockers = tuple(sorted(dependency for dependency in item.dependencies if dependency in item_map and item_map[dependency].status in FAILURE_ITEM_STATUSES))
            if blockers and item.result.get("blocked_by") != list(blockers):
                item_map[item_id] = _blocked_item(item, blockers)
                changed = True
