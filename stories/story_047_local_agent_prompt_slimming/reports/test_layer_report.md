...
- Evidence or reason: Add tests for slim default mode, full prompt-pack mode, custom prompt-file mode, context packet creation and contents, excluded review/cloud artifacts, metadata fields, length truncation warnings, empty length failure, raw response saving, artifact/public-readiness blocking, docs links, and safety boundaries.
...
- Evidence or reason: Existing CLI tests cover local-agent draft command wiring with fake runtime calls; this story adds prompt-mode argument coverage without live model calls.
...
- Evidence or reason: Existing mock E2E workflow coverage exercises story workflow; local model calls are unit-tested with fake HTTP clients.
