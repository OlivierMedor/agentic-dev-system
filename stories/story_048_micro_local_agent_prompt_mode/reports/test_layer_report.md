...
- Evidence or reason: Add tests for micro prompt mode acceptance, context packet creation, metadata fields, exclusions, required micro prompt content, micro smaller than slim, empty visible response failure, non-empty length warning behavior, prompt-file mode, and continued full and slim modes.
...
- Evidence or reason: CLI tests cover local-agent draft prompt-mode wiring with fake runtime calls.
