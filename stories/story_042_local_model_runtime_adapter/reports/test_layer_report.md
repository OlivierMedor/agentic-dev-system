...
- Evidence or reason: Add local model runtime tests for config validation, missing base_url, invalid provider, missing model, fake HTTP dry run, run-prompt output saving, source files not being applied, README link, and docs safety wording.
...
- Evidence or reason: CLI tests cover local-model validate output and local-agent run-prompt behavior without real model calls.
