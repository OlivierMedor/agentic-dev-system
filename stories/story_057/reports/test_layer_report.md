...
- Evidence or reason: Add tests for Docker Codex setup docs, disabled runtime defaults, artifact policy, public readiness, and missing-command safety.
...
- Evidence or reason: Run targeted Codex runtime and story runner tests, full pytest, and Ruff through the Docker dev container.
...
- Action: confirm_existing
- Frequency: before_merge
- Evidence or reason: Story runner tests fake Codex subprocess execution and verify safe blocking behavior without calling external services.
...
- Action: confirm_existing
- Frequency: before_merge
- Evidence or reason: Run Docker smoke checks for `which codex` and `codex --version`; these verify CLI availability only and do not call GitHub APIs or perform story execution.
...
- Evidence or reason: This story changes local Docker runtime setup only and does not deploy remote dev infrastructure.
