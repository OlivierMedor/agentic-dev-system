...
| `agentic local-execute` | Execute blueprint-selected local roles, or blueprint-defined context-safe sub-tasks when a story declares `subtasks`. | `src/agentic_dev/cli.py` | `src/agentic_dev/local_execution.py`, `src/agentic_dev/subtask_execution.py` | `tests/test_local_execution.py`, `tests/test_subtask_execution.py` | `story_060_blueprint_local_model_execution`, `story_061_blueprint_defined_context_safe_subtask_execution` best-known mapping |
| `agentic local-repair-loop` | Validate local-model outputs, classify failure evidence, build targeted repair prompts, run local-only repair retries, and write manual support evidence when the retry budget is exhausted. | `src/agentic_dev/cli.py` | `src/agentic_dev/local_repair_loop.py` | `tests/test_local_repair_loop.py` | `story_069_local_repair_loop_orchestrator` |
| `agentic demo-subtasks` | Run the Story 062 disposable calculator demo through the same sub-task execution pipeline in deterministic fake mode or configured local-runtime mode. | `src/agentic_dev/cli.py` | `src/agentic_dev/demo_subtasks.py`, `src/agentic_dev/local_execution.py`, `src/agentic_dev/subtask_execution.py` | `tests/test_demo_subtasks.py`, `tests/test_local_execution.py`, `tests/test_subtask_execution.py` | `end-to-end-blueprint-subtask-execution-demo-and-workflow-hardening` |
...
| `agentic quality-gate` | Decide whether local evidence is ready for review, or run clean-checkout post-merge verification with regenerated pytest and Ruff evidence. | `src/agentic_dev/cli.py` | `src/agentic_dev/quality_gate.py` | `tests/test_quality_gate.py` | `story_005_quality_gate`, `end-to-end-blueprint-subtask-execution-demo-and-workflow-hardening` best-known mapping |
...
| `agentic cloud-queue create` | Create a structured cloud escalation request from a local blocker. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
| `agentic cloud-queue export` | Export one request or a batch of ready requests into a manual packet. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/export.py`, `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
| `agentic cloud-queue import` | Import a manual response and classify it independently. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/importers.py`, `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
| `agentic cloud-queue approve` | Record a checksum-locked approval for a validated response. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/approvals.py`, `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
| `agentic cloud-queue reject` | Reject a cloud queue request. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
| `agentic cloud-queue status` | Summarize cloud queue state counts. | `src/agentic_dev/cli.py` | `src/agentic_dev/cloud_queue/service.py` | `tests/test_cloud_queue_service.py`, `tests/test_cloud_queue_cli.py` | `story_063_structured_cloud_escalation_and_manual_packet_queue` |
